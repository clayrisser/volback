...
func init() {
	agentCmd.Flags().StringVarP(&targetURL,
		"target.url",
		"r",
		"",
		"The target URL to push the backups to.",
	)
	agentCmd.Flags().StringVarP(&backupPath,
		"backup.path",
		"p",
		"",
		"Path to the volume to backup.",
	)
	agentCmd.Flags().StringVarP(&hostname,
		"host",
		"",
		"",
		"Custom hostname.",
	)
	agentCmd.Flags().BoolVarP(&force,
		"force",
		"",
		false,
		"Force a backup by removing all locks.",
	)
	agentCmd.Flags().StringVarP(&logReceiver,
		"log.receiver",
		"",
		"",
		"Address where the manager will collect the logs.",
	)
